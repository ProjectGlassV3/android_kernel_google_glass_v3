...
//Peter Cheng, 2018/09/06, remove WD
...
#ifdef CY_WAKE_DEBUG
        dev_info(cd->dev, "%s, rc=%d", __func__, rc);
#endif
	if (rc || status == 0)
		return -EBUSY;
...
#ifdef CY_WAKE_DEBUG
                dev_info(cd->dev, "%s: HID_WAKEUP_REPORT_ID", __func__);
#endif
		if (!si1142ps_query_status())
			cyttsp5_wakeup_host(cd);
		else
			dev_info(cd->dev, "%s: don't report wake event \
				while DOFF\n", __func__);
		return 0;
	}
...
static int cyttsp5_core_wake_(struct cyttsp5_core_data *cd)
{
	...
	//cyttsp5_start_wd_timer(cd);//Peter Cheng, 2018/09/06, remove WD
	return rc;
}
...
#ifdef CY_WAKE_DEBUG
        dev_info(dev, "%s: rc=%d", __func__, rc);
#endif
	if (rc < 0) {
...
static int cyttsp5_core_suspend(struct device *dev)
{
	...
	/* get glove mode settings */
	ret = cyttsp5_hid_output_get_param(cd, param_id, &cd->glove_mode);
	if (ret)
		dev_err(dev, "%s: get glove_mode 0x%x fail, ret: %d\n",
				__func__, cd->glove_mode, ret);
	else
		dev_info(dev, "%s, get  glove_mode: 0x%x OK\n",
...
exit:
	...
	/* set glove mode settings */
	ret = cyttsp5_hid_output_set_param(cd, param_id, cd->glove_mode, 1);
	if (ret)
		dev_err(dev, "%s: set glove_mode 0x%x fail, ret: %d\n",
				__func__, cd->glove_mode, ret);
	else
		dev_info(dev, "%s, set  glove_mode: 0x%x OK\n",
				__func__, cd->glove_mode);
		...
		struct device_attribute *attr, char *buf)
{
	...
	/* get glove mode settings */
	rc = cyttsp5_hid_output_get_param(cd, param_id, &value);
	if (rc)
		dev_err(dev, "%s: get glove_mode 0x%x fail, ret: %d\n",
				__func__, value, rc);
	else
	...
	cd->glove_mode = value;
	ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "0x%X\n",
			cd->glove_mode);
	mutex_unlock(&cd->system_lock);
	return ret;
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((FINGER_MODE != value) && (GLOVE_MODE != value))
		value = FINGER_MODE;
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	int rc = 0;
        static struct regulator *vddd_i2c;
        static struct regulator *vdda_i2c;
        int ret;

        printk("%s ++ \r\n",__func__);
        
        if (!vddd_i2c) {
		vddd_i2c = regulator_get(dev, "cy_vddd");
		if (IS_ERR(vddd_i2c)) {
			printk("%s regulator get of vddd_i2c failed\r\n",__func__);
			ret = PTR_ERR(vddd_i2c);
			vddd_i2c = NULL;
			return ret;
		}
        }
        ret = regulator_enable(vddd_i2c);
        printk("%s enable pm660_l13\r\n",__func__); 
        msleep(10);
        
        if (!vdda_i2c) {
		vdda_i2c = regulator_get(dev, "cy_vdda");
		if (IS_ERR(vdda_i2c)) {
			printk("%s regulator get of vdda_i2c failed\r\n",__func__);
			ret = PTR_ERR(vdda_i2c);
			vdda_i2c = NULL;
			return ret;
		}
        }
        ret = regulator_enable(vdda_i2c);
        printk("%s enable pm660_l15\r\n",__func__); 
	...
	//INIT_WORK(&cd->watchdog_work, cyttsp5_watchdog_work);//Peter Cheng, 2018/09/06, remove WD
	...
	/* Set glove_mode to finger_mode */
	cd->glove_mode= FINGER_MODE;
