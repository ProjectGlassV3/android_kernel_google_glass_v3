...
struct cyttsp5_loader_platform_data _cyttsp5_loader_platform_data = {
	...
	.flags = CY_LOADER_FLAG_CHECK_TTCONFIG_VERSION | CY_LOADER_FLAG_CALIBRATE_AFTER_FW_UPGRADE,//CY_LOADER_FLAG_NONE,
};
		...
		struct device *dev, cyttsp5_platform_read read)
{
	...
	while (retry--) {
		...
		parade_debug(dev, DEBUG_LEVEL_1, "%s: Performing a reset\n",
			__func__);
		...
		parade_debug(dev, DEBUG_LEVEL_1, "%s: Read unsuccessful, try=%d\n",
			__func__, 3 - retry);
	}
